...
thread_local V8Runtime* V8Runtime::tls_current_runtime_;
...
void V8Runtime::GCPrologueCallback(v8::Isolate* isolate, v8::GCType type,
  ...
  IsolateData* isolate_data =
      reinterpret_cast<IsolateData*>(isolate->GetData(ISOLATE_DATA_SLOT));
  uint32_t instance_id_ = isolate_data->instance_id_;
  V8Runtime* runtime = reinterpret_cast<V8Runtime*>(isolate_data->runtime_);

  TRACEV8RUNTIME_VERBOSE("V8::GCPrologueCallback",
                         TraceLoggingString(gcTypeString.c_str(), "GCType"));
  runtime->DumpCounters(gcTypeString.c_str(), instance_id_);
}
...
void V8Runtime::MapCounters(v8::Isolate* isolate, const char* name) {
  ...
  //    reinterpret_cast<IsolateData*>(isolate->GetData(ISOLATE_DATA_SLOT));
  // V8Runtime* runtime = reinterpret_cast<V8Runtime*>(isolate_data->runtime_);
  V8Runtime* runtime = tls_current_runtime_;

  counters_ = reinterpret_cast<CounterCollection*>(&runtime->counters_file_);
...
int* V8Runtime::LookupCounter(const char* name) {
  ...
  Counter* counter = runtime->GetCounter(name, false);
  if (counter != nullptr) {
    return counter->ptr();
  } else {
    return nullptr;
...
void* V8Runtime::CreateHistogram(const char* name, int min, int max,
  ...
  return runtime->GetCounter(name, true);
}
...
v8::Isolate* V8Runtime::CreateNewIsolate() {
  ...
  isolate_->SetData(v8runtime::ISOLATE_DATA_SLOT,
                    new v8runtime::IsolateData(
                        {foreground_task_runner_, instance_id_, this}));
...
V8Runtime::V8Runtime(V8RuntimeArgs&& args) : args_(std::move(args)) {
  ...
  v8::V8::SetUnhandledExceptionCallback(
      [](_EXCEPTION_POINTERS* exception_pointers) -> int {
        TRACEV8RUNTIME_CRITICAL_0("V8::SetUnhandledExceptionCallback");
        return 0;
      });
  ...
  if (args_.enableInspector) {
    ...
    inspector_agent_ = std::make_unique<inspector::Agent>(
        ...
        "JSIRuntime context", instance_id_, args_.inspectorPort);
