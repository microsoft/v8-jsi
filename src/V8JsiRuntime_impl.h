...
class V8Runtime : public facebook::jsi::Runtime {
 ...
 public:
  ...
  struct CounterStores {
    CounterMap* counter_map_;
    CounterCollection local_counters_;
    CounterCollection* counters_;
    char counters_file_[sizeof(CounterCollection)]{};

    CounterStores();
    Counter* GetCounter(const char* name, bool is_histogram);
    void DumpCounters(const char* when);
    void MapCounters(v8::Isolate* isolate, const char* name);
  };
 ...
 private:
  ...
  class HostObjectLifetimeTracker {
   public:
    ...
    HostObjectLifetimeTracker(V8Runtime& runtime, v8::Local<v8::Object> obj,
                              IHostProxy* hostProxy)
        : hostProxy_(hostProxy) {
      objectTracker_.Reset(runtime.GetIsolate(), obj);
      objectTracker_.SetWeak(this, HostObjectLifetimeTracker::Destroyed,
                             v8::WeakCallbackType::kParameter);
    }

    // Useful for debugging.
    ~HostObjectLifetimeTracker() { assert(isReset_); }
  ...
  class HostObjectProxy : public IHostProxy {
   private:
    static void GetInternal(std::string propName,
      ...
      if (hostObjectProxy == nullptr) std::abort();
    ...
    static void SetInternal(std::string propName, v8::Local<v8::Value> value,
                            const v8::PropertyCallbackInfo<v8::Value>& info) {
      v8::Local<v8::External> data =
          v8::Local<v8::External>::Cast(info.This()->GetInternalField(0));
      HostObjectProxy* hostObjectProxy =
   ...
   public:
    static void Get(v8::Local<v8::Name> v8PropName,
                    const v8::PropertyCallbackInfo<v8::Value>& info) {
    ...
    static void GetIndexed(uint32_t index,
                           const v8::PropertyCallbackInfo<v8::Value>& info) {
      std::string propName = std::to_string(index);
      GetInternal(propName, info);
    }

    static void Set(v8::Local<v8::Name> v8PropName, v8::Local<v8::Value> value,
                    const v8::PropertyCallbackInfo<v8::Value>& info) {
      v8::Local<v8::String> propNameStr =
          v8::Local<v8::String>::Cast(v8PropName);
    ...
    static void SetIndexed(uint32_t index, v8::Local<v8::Value> value,
                           const v8::PropertyCallbackInfo<v8::Value>& info) {
      std::string propName = std::to_string(index);
      SetInternal(propName, value, info);
    }
   ...
   private:
    ...
    void destroy() override { hostObject_.reset(); }
  ...
  class HostFunctionProxy : public IHostProxy {
   public:
    static void call(HostFunctionProxy& hostFunctionProxy,
      ...
      facebook::jsi::Value result;
      try {
        result = hostFunctionProxy.func_(runtime, thisVal, argsVector.data(),
                                         callbackInfo.Length());
      } catch (const facebook::jsi::JSError& error) {
   ...
   private:
    ...
    void destroy() override {
      func_ = [](Runtime& rt, const facebook::jsi::Value& thisVal,
  ...
  class V8PointerValue final : public PointerValue {
    ...
    ~V8PointerValue() { v8Object_.Reset(); }
 ...
 private:
  ...
  facebook::jsi::Value ExecuteString(const v8::Local<v8::String>& source,
  ...
  std::shared_ptr<CounterStores> counter_stores_;
  static thread_local std::weak_ptr<CounterStores> tls_counter_stores_;
