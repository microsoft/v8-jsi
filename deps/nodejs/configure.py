...
# v8-jsi: absolute path to the SourceLink map consumed by the v8jsi / v8jsisb
# link via /SOURCELINK: (src/v8jsi.gyp). GYP does not rebase linker
# AdditionalOptions, so an absolute path is required. The file lives in the
# v8-jsi repo src/ (this configure.py is at deps/nodejs, so ../../src) and is
# (re)generated by the v8jsi_source_link_gen GYP action.
if options.build_v8jsi or options.build_v8jsisb:
  output['variables']['v8jsi_source_link_json'] = os.path.join(
      os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
      'src', 'source_link_gen_gyp.json')
