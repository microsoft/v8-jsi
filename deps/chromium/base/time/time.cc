...
bool Time::FromStringInternal(const char* time_string,
                              bool is_local,
                              Time* parsed_time) {
  // v8-jsi fork delta: the MPL-2.0 NSPR prtime.cc that backed this parser was
  // dropped from the vendored sources (nothing in this build calls
  // Time::FromString / FromUTCString). The helper is stubbed to keep base
  // building and linking; restore prtime.cc if string-to-Time parsing is ever
  // needed here.
  (void)time_string;
  (void)is_local;
  (void)parsed_time;
