...
def _LoadToolchainEnv(cpu, toolchain_root, sdk_dir, target_store):
  ...
  if bool(int(os.environ.get('DEPOT_TOOLS_WIN_TOOLCHAIN', 1))) and sdk_dir:
    ...
    # v8-jsi: link the Spectre-mitigated MSVC CRT. vcvars prepends
    # lib\spectre\<arch> to LIB so lld-link picks the Spectre-hardened
    # vcruntime/libcmt. Unlike cl/link, clang-cl does NOT auto-link the Spectre
    # CRT even with /Qspectre, so the static CRT stays flagged as unmitigated by
    # binary-analysis tools without this. Mirrors src/v8jsi_settings.gypi.
    # Requires the VS "C++ Spectre-mitigated libs" component (present on the CI
    # image).
