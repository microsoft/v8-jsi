...
def _CopyUCRTRuntime(target_dir, source_dir, target_cpu, debug):
  ...
  for file_part in file_parts:
    ...
    # v8-jsi: skip a VC-runtime DLL whose redist source is absent. The
    # sandbox build uses the Hybrid CRT (vcruntime/msvcp linked statically; only
    # the UCRT is shared), so it never ships these redist DLLs — the copy is only
    # so dynamically-CRT-linked Chromium binaries can run. The arm64 VC redist in
    # particular is frequently not installed (a separate VS component) even when
    # the arm64 LIBS needed for *linking* are present, which would otherwise
    # break arm64 `gn gen` for no functional gain.
...
def _CopyDebugger(target_dir, target_cpu):
  ...
  #
  # v8-jsi: dbghelp.dll is OPTIONAL in this fork (upstream marks it required).
  # These debugger DLLs are only used to symbolize stacks in Chromium's own test
  # harnesses, which this narrow sbox build does not build or run. Upstream
  # requires dbghelp from `Debuggers/<target_cpu>/`, but the Windows SDK only
  # populates that per-arch folder when the separate "Debugging Tools for Windows"
  # feature is installed for that arch — commonly only x64 is present, which would
  # otherwise break x86/arm64 `gn gen` for no functional gain. Making it optional
  # lets the sandbox build gen on any agent regardless of the installed debugger
  # arches; nothing we build or ship depends on these DLLs.
